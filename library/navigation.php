...
register_nav_menus( array(
	'top-bar-r'  => esc_html__( 'Right Top Bar', 'fosterpress' ),
	'mobile-nav' => esc_html__( 'Mobile', 'fosterpress' ),
));
 ...
 */
if ( ! function_exists( 'fosterpress_top_bar_r' ) ) {
	function fosterpress_top_bar_r() {
		wp_nav_menu( array(
			'container'      => false,
			...
			'depth'          => 3,
			'fallback_cb'    => false,
			'walker'         => new fosterpress_Top_Bar_Walker(),
		));
	}
}
 ...
 */
if ( ! function_exists( 'fosterpress_mobile_nav' ) ) {
	function fosterpress_mobile_nav() {
		wp_nav_menu( array(
			'container'      => false,                         // Remove nav container
			'menu'           => __( 'mobile-nav', 'fosterpress' ),
			'menu_class'     => 'vertical menu',
			'theme_location' => 'mobile-nav',
			'items_wrap'     => '<ul id="%1$s" class="%2$s" data-accordion-menu data-submenu-toggle="true">%3$s</ul>',
			'fallback_cb'    => false,
			'walker'         => new fosterpress_Mobile_Walker(),
		));
	}
}
...
*/
if ( ! function_exists( 'fosterpress_add_menuclass' ) ) {
	function fosterpress_add_menuclass( $ulclass ) {
		$find = array('/<a rel="button"/', '/<a title=".*?" rel="button"/');
		$replace = array('<a rel="button" class="button"', '<a rel="button" class="button"');

		return preg_replace( $find, $replace, $ulclass, 1 );
	}
	add_filter( 'wp_nav_menu','fosterpress_add_menuclass' );
}
...
if ( ! function_exists( 'fosterpress_breadcrumb' ) ) {
	function fosterpress_breadcrumb( $showhome = true, $separatorclass = false ) {
		...
		if ( ! is_front_page() ) {
			...
			if ( is_single() && ! is_attachment() ) {
				...
				// Paginated archives
				echo '<li class="current item-current-' . get_query_var('paged') . '">' . __('Page', 'fosterpress' ) . ' ' . get_query_var('paged') . '</li>';
