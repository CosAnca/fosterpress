...
function fosterpress_pagination() {
	global $wp_query;
	...
	$paginate_links = paginate_links( array(
		...
		'mid_size' => 5,
		'prev_next' => true,
		'prev_text' => __( '&laquo;', 'fosterpress' ),
		'next_text' => __( '&raquo;', 'fosterpress' ),
		'type' => 'list',
	) );
...
if ( ! function_exists( 'fosterpress_menu_fallback' ) ) :
function fosterpress_menu_fallback() {
	echo '<div class="alert-box secondary">';
	/* translators: %1$s: link to menus, %2$s: link to customize. */
	printf( __( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'fosterpress' ),
		/* translators: %s: menu url */
		sprintf(  __( '<a href="%s">Menus</a>', 'fosterpress' ),
			get_admin_url( get_current_blog_id(), 'nav-menus.php' )
		),
		/* translators: %s: customize url */
		sprintf(  __( '<a href="%s">Customize</a>', 'fosterpress' ),
...
if ( ! function_exists( 'fosterpress_active_nav_class' ) ) :
function fosterpress_active_nav_class( $classes, $item ) {
	if ( $item->current == 1 || $item->current_item_ancestor == true ) {
		$classes[] = 'is-active';
	}
	return $classes;
}
add_filter( 'nav_menu_css_class', 'fosterpress_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'fosterpress_active_list_pages_class' ) ) :
function fosterpress_active_list_pages_class( $input ) {
...
add_filter( 'wp_list_pages', 'fosterpress_active_list_pages_class', 10, 2 );
endif;
...
if ( ! function_exists( 'fosterpress_responsive_video_oembed_html' ) ) :
function fosterpress_responsive_video_oembed_html( $html, $url, $attr, $post_id ) {
...
add_filter( 'embed_oembed_html', 'fosterpress_responsive_video_oembed_html', 10, 4 );
endif;
...
if ( ! function_exists( 'fosterpress_mobile_menu_id' ) ) :
function fosterpress_mobile_menu_id() {
...
if ( ! function_exists( 'fosterpress_title_bar_responsive_toggle' ) ) :
function fosterpress_title_bar_responsive_toggle() {
